import { geocodeLocation, type GeocodedPlace } from "./geocode.js";
import { getCurrentWeather, getThreeDayForecast, type CurrentWeather, type ForecastDaily, type Units } from "./weather.js";
import { getCachedValue, makeKey, setCachedValue } from "./cache.js";
import { getCurrentWeatherInsight, getForecastWeatherInsight, getWeatherQuestionInsight, type CurrentAiInsight, type ForecastAiInsight } from "./weather-descriptions.js";
...
function sectionTitle(title: string, theme: Theme): void {
  console.log(`\n${theme.accent(`[ ${title} ]`)}`);
  console.log(divider(theme));
}

function row(label: string, value: string, theme: Theme): void {
...
function parseArgs(args: string[]): ParsedArgs {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === "-u" || arg === "--units") {
      ...
      state.units = next.toLowerCase() as Units;
      i += 1;
      continue;
    }
...
function getDisplayLocation(place: GeocodedPlace): string {
  const parts = [place.name, place.admin1, place.country].filter(Boolean);
  return parts.join(", ");
}

function createTheme(useColor: boolean): Theme {
  if (!useColor) {
    return {
  ...
  return {
    ...
    label: (text: string) => `\x1b[1;33m${text}\x1b[0m`,
...
async function fetchLocationWithCache(query: string): Promise<GeocodedPlace | null> {
  const key = makeKey(["geocode", query]);
  const cached = await getCachedValue<GeocodedPlace>(key, 24 * 60 * 60 * 1000);
  if (cached) {
    return cached;
  }
...
async function fetchCurrentWeatherWithCache(place: GeocodedPlace, units: Units): Promise<{ current: CurrentWeather }> {
  const key = makeKey(["current", place.latitude, place.longitude, units]);
  const cached = await getCachedValue<{ current: CurrentWeather }>(key, 10 * 60 * 1000);
  if (
    cached
...
async function fetchForecastWithCache(place: GeocodedPlace, units: Units): Promise<{ daily: ForecastDaily }> {
  const key = makeKey(["forecast3", place.latitude, place.longitude, units]);
  const cached = await getCachedValue<{ daily: ForecastDaily }>(key, 30 * 60 * 1000);
  if (cached) {
    return cached;
  }
...
function printCurrentWeather(place: GeocodedPlace, current: CurrentWeather, units: Units, insight: CurrentAiInsight, theme: Theme): void {
...
function printForecast(place: GeocodedPlace, forecast: { daily: ForecastDaily }, insight: ForecastAiInsight, units: Units, theme: Theme): void {
...
function printQuestionAnswer(question: string, answer: string, targetDate: string | null, theme: Theme): void {
...
function printSavedLocations(items: Array<{ alias: string; location: string }>, theme: Theme): void {
...
function printDateCheck(place: GeocodedPlace, forecast: { daily: ForecastDaily }, units: Units, date: string, theme: Theme): void {
...
async function run(args: string[]): Promise<void> {
  try {
    ...
    let forecast: { daily: ForecastDaily } | null = null;
